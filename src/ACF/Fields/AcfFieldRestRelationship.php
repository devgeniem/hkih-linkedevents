...
/**
 * Class AcfFieldRestRelationship
 */
class AcfFieldRestRelationship extends \acf_field_relationship {
    ...
    public function initialize() : void {
        ...
        // extra
        add_action(
            'wp_ajax_acf/fields/rest_relationship/query',
            [ $this, 'ajax_query' ]
        );

        add_action(
            'wp_ajax_nopriv_acf/fields/rest_relationship/query',
            [ $this, 'ajax_query' ]
        );
    }
    ...
    public function render_field( $field = [] ) : void {
        ...
        $attributes = [
            'id'         => $field['id'],
            'class'      => "acf-rest-relationship acf-relationship {$field['class']}",
        ...
        <div class="selection rest_relationship_selection relationship_selection">
            <div class="choices">
     ...
     * @return array
     */
    public function get_rest_ajax_query( $options = [] ) : array {
        // defaults
        ...
        // filters
        $args = apply_filters(
            'acf/fields/rest_relationship/query',
            $args, $field, $options['post_id']
        );
        $args = apply_filters(
            'acf/fields/rest_relationship/query/name=' . $field['name'],
            $args, $field, $options['post_id']
        );
        $args = apply_filters(
            'acf/fields/rest_relationship/query/key=' . $field['key'],
            $args, $field, $options['post_id']
        );

        // We now should have all the data from 'acf/fields/rest_relationship/query' filter.
        try {
            $data = \HKIH\LinkedEvents\LinkedEventsPlugin::event_selected_fetch( $args );
        }
        ...
        // filters
        $data = apply_filters(
            'acf/fields/rest_relationship/post_rest_call',
...
acf_register_field_type( AcfFieldRestRelationship::class );
